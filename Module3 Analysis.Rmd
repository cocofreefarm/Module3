...
star <- levels(as.factor(star.dat$stars_x))

freq.data <- as.data.frame(cbind(star,freq.data))

dup.dat <- star.dat
for (i in 1:dim(star.dat)[2]-1){
  dup.dat[which(star.dat[,i+1] >=1),i+1]=1
}
dup.dat$stars_x <- star.dat$stars_x

library(nnet)
m1 <- multinom(stars_x~room+second+floor+nice+decor+furniture,data = dup.dat)
