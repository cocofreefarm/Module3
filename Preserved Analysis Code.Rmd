...
```
```{r}
facility.dat <- wordembedding.dat[c("stars","room","bathroom","pool","tub","shower","lobby","elevator","wall","parking","gym","amenity")]
```
```{r}
...
```
```{r}
m1 <- multinom(stars~.,data=service.dat)

stat <- summary(m1)$coefficient/summary(m1)$standard.errors
p.val <- 2*(pnorm(abs(stat),0,1,lower.tail = FALSE))
p.val
